...
public class Rest {
    public static <T extends Resource> T getId(Resource.ClassData resource, String id, Project user) throws Exception {
        String content = Response.fetch(Resource.endpoint(resource, id), "GET", null, null, user, "v2").content;
        JsonObject contentJson = new Gson().fromJson(content, JsonObject.class);
        JsonObject jsonObject = contentJson.get(Resource.getLastName(resource)).getAsJsonObject();
    ...
    public static <T extends Resource> List<T> post(Resource.ClassData resource, List<T> entities, Project user) throws Exception {
        ...
        payload.add(Resource.getLastPlural(resource), new Gson().toJsonTree(entities).getAsJsonArray());
        String content = Response.fetch(Resource.endpoint(resource), "POST", payload, null, user, "v2").content;
        JsonObject contentJson = new Gson().fromJson(content, JsonObject.class);
        List<T> postEntities = new ArrayList<>();
        JsonArray jsonArray = contentJson.get(Resource.getLastPlural(resource)).getAsJsonArray();
        ...
        return new Generator<T>() {
            public void run() throws Exception {
                ...
                do {
                    ...
                    String content = Response.fetch(Resource.endpoint(resource), "GET", null, params, user, "v2").content;
    ...
    public static InputStream getPdf(Resource.ClassData resource, String id, Project user) throws Exception {
        return Response.fetchStream(Resource.endpoint(resource, id) + "/pdf", "GET", null, null, user, "v2");
    }

    public static <T extends Resource> T delete(Resource.ClassData resource, String id, Project user) throws Exception {
        String content = Response.fetch(Resource.endpoint(resource, id), "DELETE", null, null, user, "v2").content;
        Gson gson = new GsonBuilder()
                .registerTypeAdapter(Event.class, new Event.Deserializer())
                .setDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ")
                .create();
